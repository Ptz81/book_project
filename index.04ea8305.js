 ...
 * limitations under the License.
 */
class Nf{constructor(){this.eventLists_=[],this.recursionDepth_=0}}function Sf(e,t){let n=null;for(let i=0;i<t.length;i++){const r=t[i],o=r.getPath();null===n||_u(o,n.path)||(e.eventLists_.push(n),n=null),null===n&&(n={events:[],path:o}),n.events.push(r)}n&&e.eventLists_.push(n)}function Rf(e,t,n){Sf(e,n),Af(e,(e=>bu(e,t)||bu(t,e)))}function Af(e,t){e.recursionDepth_++;let n=!0;for(let i=0;i<e.eventLists_.length;i++){const r=e.eventLists_[i];if(r){t(r.path)?(Lf(e.eventLists_[i]),e.eventLists_[i]=null):n=!1}}n&&(e.eventLists_=[]),e.recursionDepth_--}function Lf(e){for(let t=0;t<e.events.length;t++){const n=e.events[t];if(null!==n){e.events[t]=null;const i=n.getEventRunner();wc&&Cc("event: "+n.toString()),Bc(i)}}}
/**
 * @license
 * Copyright 2017 Google LLC
 *
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */();const xp="Anonymous";let kp,Cp,Ip,Ep;const Tp=new class{constructor(e){if(er)return er;Zi.addEventListener("click",(()=>er.hide())),Ki.addEventListener("change",or),Qi.addEventListener("change",(({currentTarget:e})=>{Ji.type=e.checked?"text":"password"})),addEventListener("resize",rr);const t=ir(e)||ir("signin");nr=new En({child:Xi,hideOnClick:!0}),this.hide=nr.hide.bind(nr),this.reset=Xi.reset.bind(Xi),this.show=function(e){ir(e||t),rr(),Ji.type="password",nr.show()},er=this}get zindex(){return nr.zindex}get mode(){return Xi.formMode.value}onSubmit(e){tr=Vi(e)?e:null,Xi.addEventListener("submit",(e=>{e.preventDefault();const t=e.currentTarget;tr&&tr(function(e){return Array.from(new FormData(e)).reduce(((e,[t,n])=>(e[t]=n,e)),{})}(t),t)}))}},Np=new class{constructor({app:e}={}){if(sc)return sc;try{ac=e||os(cr),oc=ic(ac)}catch{return console.error("FirebaseApp initialization failed"),null}var t,n,i;t=e=>{if(e)return lc&&lc(e);cc&&cc()},vo(oc).onAuthStateChanged(t,n,i),sc=this}get app(){return ac}get currentUser(){return oc.currentUser}onSignedIn(e){lc=bn(e)?e:null}onSignedOut(e){cc=bn(e)?e:null}async signUp({email:e,password:t}={}){try{return await Ja(oc,e,t)}catch(e){throw new uc(e)}}async signIn({email:e,password:t}={}){try{return await Qa(oc,e,t)}catch(e){throw new uc(e)}}async signOut(){try{return await(e=oc,vo(e).signOut())}catch(e){throw new uc(e)}var e}},Sp=new class{constructor({app:e}={}){if(vp)return vp;try{wp=e||os(cr),_p=yp(wp),bp=lp(_p)}catch{return console.error("FirebaseApp initialization failed"),null}vp=this}get app(){return wp}async write(e,t){try{return await up(lp(_p,e),t)}catch(e){console.error(e)}}async read(e){try{const t=await hp(cp(bp,e));return t.exists()?t.val():null}catch(e){console.error(e)}}}(Np),Rp=new Rn;function Ap({isNewcomer:e,name:t}){Bn(`${e?"Glad to welcome you":"Welcome back"}, ${t}`)}function Lp(){Cp&&(Bn(`See you soon, ${Cp.name}`),Ep&&Ep(Cp),Cp=null)}async function Op(e){try{Rp.show({zindex:Tp.zindex+1}),await("signup"===Tp.mode?async function(e,t){const{name:n,email:i}=e,r=(await Np.signUp(e)).user.uid,o=`users/${r}`;await Sp.write(o,{name:n,email:i,shoppingList:null}),Cp={name:n,email:i,id:r,isNewcomer:!0},bn(t)&&t(Cp)}(e,Ap):async function(e,t){const{name:n=xp,email:i}=e,r=(await Np.signIn(e)).user.uid,o=`users/${r}`,s=await Sp.read(o);s||await Sp.write(o,{name:n,email:i,shoppingList:null});Cp={name:n,email:i,id:r,...s},bn(t)&&t(Cp)}(e,Ap)),Ip&&Ip(Cp),Tp.hide(),Tp.reset()}catch(e){Un(e,{zindex:Tp.zindex})}finally{Rp.hide()}}new Rn;const Pp=new class{constructor(){if(kp)return kp;Tp.onSubmit(Op),Np.onSignedOut(Lp),this.showForm=Tp.show.bind(Tp),this.hideForm=Tp.hide.bind(Tp),this.logout=Np.signOut.bind(Np),kp=this}get currentUser(){return Cp}onLogin(e){Ip=bn(e)?e:null}onLogout(e){Ep=bn(e)?e:null}setShoppingList(e){const{id:t}=Cp||"";t&&Sp.write(`users/${t}/shoppingList`,e)}},Dp=document.querySelector(".header-btn"),Mp=document.querySelector(".nav .shop-link"),Fp=(e=!0)=>Mp.style.display=e?"none":"block";Fp(),Dp.textContent="Sign In",Dp.addEventListener("click",(e=>{Pp.currentUser?Pp.logout():Pp.showForm()})),Pp.onLogin((e=>{Fp(!1),Dp.textContent=e.name})),Pp.onLogout((()=>{Fp(),Dp.textContent="Sign In"}));
//# sourceMappingURL=index.04ea8305.js.map
